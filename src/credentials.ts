...
import { hostOpenCommand } from '@madarco/agentbox-provider-sdk';
...
import { writeManagedSecrets, type CredSetResult } from './secrets.js';
...
export interface EnsureTenkiCredentialsOptions {
  /** Re-prompt even when valid credentials are already present. */
  force?: boolean;
}
...
function persistCredentials(creds: { authToken: string }): void {
  // Atomic writer (temp-file + rename, mode 0600) so a token is never half
  // written or briefly world-readable. See ./secrets.ts.
  writeManagedSecrets(MANAGED_KEYS, { TENKI_AUTH_TOKEN: creds.authToken });
}

/**
 * Non-interactive credential write — the headless path an API driver uses,
 * bypassing the TTY-gated `ensureTenkiCredentials` prompts. Extracts the `token`
 * field, persists it, and returns a normalized result without echoing the secret.
 */
